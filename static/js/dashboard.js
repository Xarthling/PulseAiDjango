...
// Use a singleton pattern for DashboardState
const DashboardState = (() => {
    ...
    return {
        getState: () => state,
        setCharts: (charts) => state.charts = charts,
        setFilters: (filters) => state.filters = filters,
        setInitialData: (data) => state.initialData = data,
        getFromCache: (key) => state.cache.get(key),
        setToCache: (key, value) => state.cache.set(key, value)
    };
})();
...
async function initializeDashboard(initialData) {
    ...
    try {
        DashboardState.setInitialData(initialData);
...
function createOrUpdateChart(id, type, data, options = {}) {
    ...
    if (DashboardState.getState().charts.has(id)) {
        const chart = DashboardState.getState().charts.get(id);
        chart.data = data;
    ...
    DashboardState.getState().charts.set(id, chart);
    return chart;
}

// Optimize chart creation with memoization
const memoizedCreateChart = (() => {
    const cache = new Map();
    
    return (id, type, data, options) => {
        const cacheKey = JSON.stringify({ id, type, data });
        if (cache.has(cacheKey)) {
            return cache.get(cacheKey);
        }
        
        const chart = createOrUpdateChart(id, type, data, options);
...
function createCategoryChart(data) {
    ...
    memoizedCreateChart('categoryChart', 'bar', chartData, {
        plugins: { 
...
function createSeasonChart(data) {
    ...
    memoizedCreateChart('seasonChart', 'bar', chartData, {
        plugins: { 
            legend: { display: false },
            tooltip: {
                callbacks: getDefaultTooltipCallback()
...
function createLocationChart(data) {
    ...
    memoizedCreateChart('locationChart', 'bar', chartData, {
        indexAxis: 'y',
        plugins: { 
            legend: { display: false },
            tooltip: {
...
function createSalesTrendChart(data, period = 'month') {
    ...
    memoizedCreateChart('salesTrendChart', 'line', chartData, {
        plugins: { 
            legend: { display: false },
            tooltip: {
                mode: 'index',
...
function createRegionChart(data) {
    ...
    memoizedCreateChart('regionChart', 'pie', chartData, {
        plugins: {
...
function createStoreSizeChart(data) {
    ...
    memoizedCreateChart('storeSizeChart', 'bar', chartData, {
        indexAxis: 'y',
        plugins: { 
            legend: { display: false },
            tooltip: {
...
function createTopSellingProductsChart(data) {
    ...
    memoizedCreateChart('topSellingProductsChart', 'bar', chartData, {
...
function createCrossSellUpsellChart(data) {
    ...
    memoizedCreateChart('crossSellUpsellChart', 'bar', chartData, {
        indexAxis: 'y',
        plugins: {
            legend: { display: false },
            tooltip: {
...
function createCLVDistributionChart(data) {
    ...
    memoizedCreateChart('clvDistributionChart', 'bar', chartData, {
        plugins: { legend: { display: false } },
        scales: { y: { beginAtZero: true } }
    });
}
...
function createDiscountImpactChart(data) {
    ...
    memoizedCreateChart('discountImpactChart', 'bar', chartData, {
        scales: {
            y1: {
                ...getDefaultScaleConfig(true),
                type: 'linear',
...
function createAgeDistributionChart(data) {
    ...
    memoizedCreateChart('ageChart', 'bar', chartData, {
        plugins: { legend: { display: false } },
        scales: { y: { beginAtZero: true } }
    });
}
...
function createGenderDistributionChart(data) {
    ...
    memoizedCreateChart('genderChart', 'pie', chartData, {
        plugins: {
            legend: getLegendConfig(true, 'right', data),
            tooltip: {
                callbacks: {
...
function createAgeBinsChart(data) {
    ...
    memoizedCreateChart('ageBinsChart', 'pie', chartData, {
        plugins: {
            legend: getLegendConfig(true, 'right', data),
            tooltip: {
                callbacks: {
...
function createPeakHoursChart(data) {
    ...
    memoizedCreateChart('peakHoursChart', 'line', chartData, {
        plugins: { 
            legend: { display: false },
            tooltip: {
                callbacks: {
...
function createPromoCodeChart(data) {
    ...
    memoizedCreateChart('promoCodeChart', 'doughnut', chartData, {
        plugins: {
            legend: getLegendConfig(true, 'right', data),
            tooltip: {
                callbacks: {
...
function createDiscountHistogramChart(data) {
    ...
    memoizedCreateChart('discountHistogramChart', 'bar', chartData, {
        plugins: { legend: { display: false } },
        scales: {
            y: { beginAtZero: true },
...
function createRFMSegmentsChart(data) {
    ...
    memoizedCreateChart('rfmSegmentsChart', 'bar', chartData, {
        plugins: { legend: { display: false } },
        scales: { y: { beginAtZero: true } }
    });
}
...
function createVisitVsPurchaseChart(data) { 
    ...
    memoizedCreateChart('visitVsPurchaseFrequencyChart', 'scatter', chartData, options);
}
...
function createSalesByDayChart(data) {
    ...
    memoizedCreateChart('salesByDayChart', 'bar', chartData, {
        plugins: {
            legend: {
                display: true,
                position: 'top'
...
function initializeStoreSalesTable() {
    ...
    if (DashboardState.getState().initialData && DashboardState.getState().initialData.graphs.sales_by_store) {
        sortAndDisplayData(DashboardState.getState().initialData.graphs.sales_by_store, 'desc');
    }
}
...
function createPredictedSalesChart(data) {
    ...
    memoizedCreateChart('predictedSalesChart', 'line', chartData, {
        scales: {
            y: {
                beginAtZero: true,
                title: {
...
const API = {
    ...
    getCSRFToken() {
        // First try getting from window variable
    ...
    // Update existing methods to use the new helper
    ...
    batchRequests() {
        ...
        return Promise.all(requests.map(req => this.fetchFilteredData(req.filters)))
            .then(results => {
    ...
    queueRequest(filters) {
        return new Promise((resolve, reject) => {
            this.requestQueue.push({ filters, resolve, reject });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('applyFilter')
        .addEventListener('click', debouncedHandleFilterApply);
...
function updateChartTheme() {
    ...
    DashboardState.getState().charts.forEach(chart => {
        if (!chart) return;
...
const debouncedHandleFilterApply = debounce(handleFilterApply, 250);
const throttledUpdateChartTheme = throttle(updateChartTheme, 100);
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const chartData = DashboardState.getFromCache(chartId);
                if (chartData) {
                    createOrUpdateChart(chartId, chartData.type, chartData.data, chartData.options);
...
// Update handleGenerateReport to include better error handling
function handleGenerateReport() {
    ...
    try {
        ...
        const cards = {
            'Total Sales': extractNumericValue($(".card:contains('Total Sales') h2").text()),
            'Total Transactions': extractNumericValue($(".card:contains('Total Transactions') h2").text()),
            'Average Sales': extractNumericValue($(".card:contains('Average Sales') h2").text()),
            'Average Rating': extractNumericValue($(".card:contains('Average Rating') h2").text())
        };
        ...
        DashboardState.getState().charts.forEach(chart => {
            if (chart) {
                try {
                    ...
                    graphs.push(base64Image.replace(/^data:image\/(png|jpg);base64,/, ""));
                } catch (err) {
        ...
        if (graphs.length === 0) {
            throw new Error('No charts available for report generation');
        }

        API.generateReport({ filters, cards, graphs })
            .then(response => {
                const reportUrl = `${window.location.origin}${window.STATIC_URL || '/static/'}${response.report_path}`;
